...
export type Block = {
  ...
  rpe: number | null;
  // DEPRECATO: un solo punteggio per blocco. Mantenuto solo per leggere le
  // schede salvate prima dell'introduzione di più punteggi per blocco: le
  // nuove schede usano `scores`. Non scrivere più in questo campo — usa
  // sempre `getBlockScores()` per leggere, sia sul vecchio che sul nuovo
  // formato.
  score: ScoreConfig | null;
  // Elenco dei punteggi richiesti per questo blocco (es. peso E ripetizioni
  // per lo stesso esercizio). Sostituisce `score`.
  scores?: ScoreConfig[];
...
// Ritorna sempre l'elenco dei punteggi richiesti per un blocco, sia che
// siano stati salvati nel formato nuovo (scores: ScoreConfig[]) sia nel
...
// Chiave usata in ClientScores per un dato punteggio di un blocco. Il primo
// punteggio di ogni blocco (scoreIndex 0) usa la stessa chiave di sempre
// (solo l'indice del blocco), così le schede già compilate restano leggibili
// senza bisogno di migrazione: continuano a corrispondere al "punteggio 0"
// anche ora che un blocco può averne più di uno. I punteggi aggiuntivi
// (scoreIndex 1, 2, ...) usano una chiave composta "blocco:punteggio".
export function clientScoreKey(blockIndex: number, scoreIndex: number): string {
  return scoreIndex === 0 ? String(blockIndex) : `${blockIndex}:${scoreIndex}`;
}

// Legge il punteggio inserito dal cliente per un dato blocco+punteggio,
// gestendo la chiave composta di cui sopra.
export function readClientScoreEntry(
  scores: ClientScores | null | undefined,
  blockIndex: number,
  scoreIndex: number
